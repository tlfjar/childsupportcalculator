      ...
      >
        ...
        >
          ...
          <div class="row q-mx-sm q-my-xs text-justify">
            ...
            gross income of ${{ (+GrossIncome.plaintiff).toFixed(2) }} per month
            and {{ PayorPayee.payee }} (payee) earns a gross income of ${{
              (+GrossIncome.defendant).toFixed(2)
            }}
            per month. Therefore, the parents’ combined gross income is ${{
              (+GrossIncome.combined).toFixed(2)
            }}
            ...
            court also finds that {{ Expenses.sentence }}. Plaintiff ({{
              PayorPayee.plaintiff
            }}) is responsible for
            {{ (GrossPercentage.plaintiff * 100).toFixed(0) }}% of the total
            obligation (${{ (+GrandTotals.psharebasic).toFixed(2) }} share of
            basic obligation plus ${{
              (+GrandTotals.pshareexpenses).toFixed(2)
            }}
            for expenses) and has a total child-support obligation of ${{
              (+GrandTotals.psharetotal).toFixed(2)
            }}. Defendant ({{ PayorPayee.defendant }}) is responsible for
            {{ (GrossPercentage.defendant * 100).toFixed(0) }}% of the total
            obligation (${{ (+GrandTotals.dsharebasic).toFixed(2) }} share of
            basic obligation plus ${{
              (+GrandTotals.dshareexpenses).toFixed(2)
            }}
            for expenses) and has a total child-support obligation of ${{
              (+GrandTotals.dsharetotal).toFixed(2)
            }}. {{ PayorPayee.payor }}, as the payor, shall receive a ${{
              (+GrandTotals.payorexpenses).toFixed(2)
            }}
          ...
          >
            <DocxWorksheet
              :plaintiff="{
                ...
                presumedsupport:
                  +GrandTotals.psharetotal - +Expenses.ptotalexpenses,
              }"
              :defendant="{
                name: defendantName,
                ...
                presumedsupport:
                  +GrandTotals.dsharetotal - +Expenses.dtotalexpenses,
              }"
    ...
    Expenses() {
      ...
      if (dtotalexpenses > 0) {
        if (this.dHealthInsurance > 0) {
          dhi = `\$${(+this.dHealthInsurance).toFixed(2)} in health insurance`;
          dt = `${dt}A`;
        } else {
          dhi = "";
        }
        if (this.dMedicalExpenses > 0) {
          dme = `\$${(+this.dMedicalExpenses).toFixed(
            2
      ...
      if (ptotalexpenses > 0) {
        if (this.pHealthInsurance > 0) {
          phi = `\$${(+this.pHealthInsurance).toFixed(2)} in health insurance`;
          pt = `${pt}A`;
        } else {
          phi = "";
        }
        if (this.pMedicalExpenses > 0) {
          pme = `\$${(+this.pMedicalExpenses).toFixed(
            2
          )} in extraordinary medical expenses`;
          pt = `${pt}B`;
        } else {
